...
/// 分析磁盘碎片情况
#[tauri::command]
pub fn analyze_defrag() -> Vec<DriveDefragInfo> {
...
/// 执行磁盘碎片整理 (HDD) 或 TRIM 优化 (SSD)
#[tauri::command]
pub fn run_defrag(drive: String, optimize_ssd: bool) -> DefragResult {
...
/// 对所有磁盘执行 TRIM (SSD 优化)
#[tauri::command]
pub fn run_trim_all() -> DefragResult {
    let start = std::time::Instant::now();
