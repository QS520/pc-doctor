...
/// 获取完整系统信息（异步执行，避免阻塞 UI 主线程）
#[tauri::command]
pub async fn get_system_info() -> SystemInfoResult {
    tokio::task::spawn_blocking(compute_system_info)
        .await
        .unwrap_or_default()
}

/// 实际计算逻辑（在独立线程执行，内部包含 PowerShell 调用）
fn compute_system_info() -> SystemInfoResult {
...
/// 获取 CPU 温度 (通过 WMI 查询) —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn get_cpu_temperature() -> Result<f32, String> {
