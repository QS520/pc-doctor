...
/// 获取所有电源计划
#[tauri::command]
pub async fn get_power_plans() -> Vec<PowerPlan> {
...
/// 切换电源计划
#[tauri::command]
pub async fn set_power_plan(guid: String) -> PowerPlanResult {
    tokio::task::spawn_blocking(move || {
    #[cfg(windows)]
    {
        let output = Command::new("powercfg")
...
/// 获取 CPU 降频状态
#[tauri::command]
pub async fn get_cpu_throttle_info() -> CpuThrottleInfo {
    tokio::task::spawn_blocking(move || {
    #[cfg(windows)]
    {
        // 获取当前活动电源方案
...
#[tauri::command]
pub async fn set_cpu_max_state(percent: u32) -> PowerPlanResult {
    tokio::task::spawn_blocking(move || {
    #[cfg(windows)]
    {
        let output = Command::new("powercfg")
