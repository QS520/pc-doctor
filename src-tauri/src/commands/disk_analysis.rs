...
/// 扫描指定目录下的大文件 (>100MB) —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn scan_large_files(path: Option<String>, min_size_mb: Option<f64>) -> ScanResult {
...
/// 扫描指定目录下各子目录的大小 —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn scan_directory_sizes(path: Option<String>) -> Vec<DirectorySize> {
    tokio::task::spawn_blocking(move || {
    let scan_path = path.unwrap_or_else(|| "C:\\".to_string());
...
#[tauri::command]
#[allow(dead_code)]
pub async fn delete_directory(path: String) -> Result<bool, String> {
    tokio::task::spawn_blocking(move || {
        let path_buf = PathBuf::from(&path);
    if !path_buf.exists() {
