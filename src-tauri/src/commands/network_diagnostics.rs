...
/// Ping 测试
#[tauri::command]
pub async fn ping_test(host: Option<String>) -> PingResult {
...
/// DNS 解析测速
#[tauri::command]
pub async fn dns_test(domain: Option<String>, dns_server: Option<String>) -> DnsTestResult {
    tokio::task::spawn_blocking(move || {
...
/// 路由追踪 (tracert)
#[tauri::command]
pub async fn traceroute(host: Option<String>, max_hops: Option<u32>) -> Vec<TraceRouteHop> {
    tokio::task::spawn_blocking(move || {
    let target = host.unwrap_or_else(|| "8.8.8.8".to_string());
