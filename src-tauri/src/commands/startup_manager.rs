...
/// 禁用启动项 (将其从 Run 移到 Run- 下，或从启动文件夹删除快捷方式) —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn disable_startup_item(name: String, source: String) -> Result<bool, String> {
...
/// 启用启动项 (从 Run- 恢复到 Run) —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn enable_startup_item(name: String, source: String) -> Result<bool, String> {
    tokio::task::spawn_blocking(move || {
    #[cfg(windows)]
    {
        match source.as_str() {
...
/// 获取上次开机时长 (通过事件日志 100-109 启动事件) —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn get_boot_duration() -> BootDuration {
    tokio::task::spawn_blocking(|| {
    #[cfg(windows)]
    {
