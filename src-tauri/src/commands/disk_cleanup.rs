...
/// 扫描逻辑（在阻塞线程执行）
fn scan_junk_files_impl() -> ScanResult {
...
/// 扫描所有可清理的垃圾文件 —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn scan_junk_files() -> ScanResult {
    tokio::task::spawn_blocking(scan_junk_files_impl)
        .await
        .unwrap_or_default()
}

/// 清理选定的垃圾文件 —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn clean_junk_files(category_ids: Vec<String>) -> CleanResult {
    ...
    let all_categories = scan_junk_files_impl();
...
/// 清空回收站 —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn empty_recycle_bin() -> Result<bool, String> {
...
/// 获取磁盘剩余空间 —— 异步执行避免阻塞 UI
#[tauri::command]
pub async fn get_disk_space(drive: Option<String>) -> DiskSpace {
    tokio::task::spawn_blocking(move || {
    #[cfg(windows)]
    {
